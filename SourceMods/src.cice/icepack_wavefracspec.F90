...
!
      subroutine icepack_step_wavefracture(wave_spec_type,   &
                  ...
                  trcrn,         d_afsd_wave,  divu, floe_binwidth, hin_max,d_afsd_nils)
      ...
      real (kind=dbl_kind), intent(in) :: &
         ...
         vice, divu            ! ice volume per unit area
...
!      if ((aice > p01).and.(MAXVAL(wave_spectrum(:)) > puny)) then
      ...
      call correct_FSD (ncat, nfsd, trcrn(nt_fsd:nt_fsd+nfsd-1,:), aicen, divu, floe_rad_c, floe_binwidth,floe_rad_l,dt, hin_max, d_afsd_nils)
                  call icepack_cleanup_fsd (ncat, nfsd, trcrn(nt_fsd:nt_fsd+nfsd-1,:))
