...
!          Elizabeth C. Hunke, LANL
      ...
      use ice_arrays_column, only: wave_spectrum, &
          d_afsd_wave, floe_rad_l, floe_rad_c, wavefreq, dwavefreq, floe_binwidth, hin_max, d_afsd_nils
      ...
      do iblk = 1, nblocks
         ...
         do i = ilo, ihi
            ...
            call icepack_step_wavefracture (wave_spec_type, &
                                            ...
                                            wavefreq(:),   dwavefreq(:),   &
                                            trcrn          (i,j,:,:,iblk), &
                                            d_afsd_wave    (i,j,:,  iblk),  divu(i,j,iblk), floe_binwidth(:),hin_max(:),d_afsd_nils(i,j,:,  iblk))
            afsdn       = trcrn(i,j,nt_fsd:nt_fsd+nfsd-1,:,iblk)
            do nc = 1, ncat
               tot = c0
               do k = 1, nfsd
