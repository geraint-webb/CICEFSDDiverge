...
!      call icepack_warnings_add(warnstr)  
       ...
       do n=1, ncat
          last_cat_hold=1
          do k=1, nfsd
             if (last_cat_location(k,n)==.true.) then ! Will also need acaptive timestep...
                last_cat_hold=k
 ...
 do n=1, ncat
    if (sum(trcrn(:,n))>puny .and. (sum(trcrn(:,n))>1.03*sum(int_afsd(:,n)) .or. sum(trcrn(:,n))<sum(0.97*int_afsd(:,n)))) then
