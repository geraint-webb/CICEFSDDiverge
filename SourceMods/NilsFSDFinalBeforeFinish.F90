...
module NilsFSD
  ...
  use icepack_kinds
    ...
    contains
      ...
      real (kind=dbl_kind) :: ice_remaining,tol,factor, divu_per_day, tester, icelost, weight, time_steps_per_day, max_cat_value, floe_diameter, floe_increment, a0_search, d0_search_last, dP_search, dP_search_deriv, min_bnd, max_bnd, diam, dc, di,point1,point2, ai, ac, dP_last!d0_weighted_sum, d0_search_sum, d0_weighted\
...
!      call icepack_warnings_add(warnstr)  
       ...
       do n=1, ncat
         ...
         call icepack_warnings_add(warnstr)
          ...
          run_ice_cat(n)=run_ice_cat(n)+icelost+(icelost/-new_dP(last_cat(n)))*sum(dP_saved(last_cat(n):ubound(dP_saved, 1)-1,n))
